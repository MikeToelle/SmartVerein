...
class EditLoginValidator extends \TYPO3\CMS\Extbase\Validation\Validator\AbstractValidator
{
    /**
     * Object Manager
     *
    ...
    public function isValid($member) {
        ...
        $oldMember = $this->objectManager->get('Typo3graf\\VmBase\\Service\\SessionHandler')->restoreFromSession('edit'.$member->getUid());
        $oldUser = $oldMember->getMemberLogin();

        if (($this->frontendUserRepository->findByUsername($user->getUsername())->count() > 0)&&($user->getUsername())!= $oldUser->getUsername()){
